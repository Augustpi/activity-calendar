import { colorShade, groupByData } from "./utils/helpers";
/**
 * getDayName
 * @param {*} dateStr
 * @param {*} locale
...
const activity = (day, arr) => {
  ...
  if (arr === undefined)
    dayEl = `<div class='a__day title'>${days[day].substr(0, 3)}</div>`;
  const groupByArr = groupByData(arr);
  arr = uniqueElements(arr);

  if (arr.length > 0)
    for (var i = 0; i < arr.length; i++)
      if (arr[i] == day) {
        dayEl = `<div class='a__selday item-danger' style="background-color: ${colorShade(
          "#00fe00",
          -(groupByArr[arr[i]] * 30)
        )}"></div>`;
      }
...
function activityCalendar(year, month, activeDays = []) {
  ...
  var endOftheMonthDay = numberOfTheFirstDayOfTheMonth(
  ...
  var wholeWeek =
    (countMonthDays(month, year) + firstWeekGap + lastWeekGap) / 7 - 2;
  var dayElSec = '<div class="a__selday"></div>';
  var dayOutsideOfTheMonth = '<div class="a__day is-outside-month"></div>';
  var monthNameEl = `<div class="a__label">  ${
    month ? months[month - 1] : "m"
  } ${year.toString().substr(2, 2)} </div>`;
  ...
  if (year === 0 && month === 0) {
    ...
    [firstWeekFilllerDiv, dayOftheWeek] = createWeek(
      firstWeekFilllerDiv,
      7 - firstWeekGap,
      activeDays
    );
    ...
    // Last week
    [lastWeekFilllerDiv, dayOftheWeek] = createWeek(
      lastWeekFilllerDiv,
      endOftheMonthDay,
      activeDays
    );
    ...
    } else if ((lastWeekGap = 0)) lastWeekGaplerDiv = "";
...
function generate(calendar, key) {
  for (let i in calendar)
    for (let m in calendar[i]) activityCalendar(key, m, calendar[i][m]);
}
